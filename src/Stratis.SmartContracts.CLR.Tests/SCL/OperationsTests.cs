...
namespace Stratis.SmartContracts.CLR.Tests.SCL
{
    public class OperationsTests
    {
        ...
        [Fact]
        public void CanFlattenArray()
        {
            byte[] res = Operations.FlattenArray(new [] { new byte[] { 1, 2, 3 }, new byte[] { 4, 5, 6 }, new byte[] { 7, 8, 9 } }, 3);

            Assert.NotNull(res);
            Assert.Equal(9, res.Length);
            Assert.Equal(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 9 }, res);
        }
    }
}
