    ...
    public class CoinViewTests
    {
        ...
        public void TestDatabaseSerialization()
        {
            using (NodeContext ctx = NodeContext.Create(this))
            {
                ...
                ctx.Coindb.SaveChanges(new UnspentOutput[] { new UnspentOutput(new OutPoint(genesis.Transactions[0], 0), new Coins(0, genesis.Transactions[0].Outputs.First(), true)) }, new Dictionary<TxDestination, Dictionary<uint, long>>(), new HashHeightPair(genesisChainedHeader), new HashHeightPair(chained), new List<RewindData>());
                ...
                chained = this.MakeNext(this.MakeNext(genesisChainedHeader, ctx.Network), ctx.Network);
                ctx.Coindb.SaveChanges(new List<UnspentOutput>(), new Dictionary<TxDestination, Dictionary<uint, long>>(), new HashHeightPair(previous), new HashHeightPair(chained), new List<RewindData>());
