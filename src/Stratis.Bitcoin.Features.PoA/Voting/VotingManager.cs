...
namespace Stratis.Bitcoin.Features.PoA.Voting
{
    public class VotingManager : IDisposable
    {
        ...
        private void CleanFinishedPollsLocked()
        {
            ...
            // executed polls here.
            List<Poll> finishedPolls = this.GetFinishedPolls().Where(x => !x.IsExecuted).ToList();
        ...
        private void OnBlockConnected(BlockConnected blockConnected)
        {
            try
            {
                ...
                lock (this.locker)
                {
                    foreach (Poll poll in this.GetFinishedPolls().Where(x => x.PollVotedInFavorBlockData.Hash == newFinalizedHash).ToList())
                    {
